...
def peak_summit_near_bin_center(task_name,task_bed,args,non_zero_bins):
    '''
    ...
    for entry in task_bed:
        chrom=entry[0]
        peak_start=entry[1]
        summit=peak_start+entry[-1]
        
        min_bin_center_start=floor((summit-args.bin_center_size)/args.stride)*args.stride
        ...
        for(bin_center_start in range(min_bin_center_start,max_bin_center_start+1,args.stride)):
            ...
            cur_bin_end=bin_center_start+args.bin_size 
...
def peak_percent_overlap_with_bin(task_name,task_bed,args,non_zero_bins):
    for entry in task_bed:
        chrom=entry[0]
        peak_start=entry[1]
        peak_end=entry[2]
